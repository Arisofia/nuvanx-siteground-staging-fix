...
/**
 * Keep the EXION comparison table contained on narrow viewports.
 *
 * The public style stack registers the `nvx-patterns` handle even when the
 * source files are inlined, so this addition is folded into the same governed
 * critical bundle instead of creating another stylesheet request.
 */
function nvx_exion_compare_table_styles(): void {
	if ( ! is_page() || 'exion-btl' !== (string) get_post_field( 'post_name', get_queried_object_id() ) ) {
		return;
	}

	$css = <<<'CSS'
.nvx-exion-compare-wrap {
  width: 100%;
  overflow-x: auto;
  ...
  background: var(--nvx-surface-base, var(--nvx-color-paper));
}

.nvx-exion-compare-table {
  width: 100%;
  min-width: 0;
  border-collapse: collapse;
  font-family: var(--nvx-sans);
  font-size: var(--nvx-type-body-sm);
  line-height: var(--nvx-lh-body-sm);
  color: var(--nvx-ink);
}

.nvx-exion-compare-table th,
.nvx-exion-compare-table td {
  padding: var(--nvx-space-3) var(--nvx-space-4);
  border-bottom: var(--nvx-border-hairline) solid var(--nvx-color-line);
  vertical-align: top;
}

.nvx-exion-compare-table thead th {
  background: var(--nvx-surface-soft, var(--nvx-color-soft));
  font-weight: var(--nvx-fw-semibold);
  text-align: left;
  letter-spacing: var(--nvx-track-wide);
  color: var(--nvx-ink);
}

.nvx-exion-compare-table tbody th[scope="row"] {
...
.nvx-exion-compare-table tbody td {
  color: var(--nvx-ink-muted);
}

.nvx-exion-compare-table tbody tr:last-child th,
.nvx-exion-compare-table tbody tr:last-child td {
  border-bottom: none;
}
CSS;

	wp_add_inline_style( 'nvx-patterns', $css );
}
add_action( 'wp_enqueue_scripts', 'nvx_exion_compare_table_styles', 20 );
